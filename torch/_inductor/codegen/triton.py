...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def codegen_kernel(self, name=None) -> str:
        ...
        # compilation with known shapes. Also disabled when autotune_at_compile_time
        # is enabled, since the variant dispatch path in wrapper.py does not handle
        # the autotune example args generation that autotune_at_compile_time requires.
        # Also disabled for WrapperFxCodegen which cannot represent if/else dispatch
        # in FX IR (codegen_fx is not implemented for VariantKernelCallLine).
